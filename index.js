...
function checkDevicesUptime(devices) {
  ...
  for (let device of devices) {
    // Push to array
    let diffMinutes = moment
      .duration(moment().diff(moment(device.DateTime)))
      .asMinutes();
    if (
      diffMinutes >= parseInt(device.KeepAliveMinutes) &&
      diffMinutes <= parseInt(device.KeepAliveMinutes) + 5
    ) {
      bulk.push({
  ...
  if (bulk.length > 0) {
    ...
    bulk.forEach((device) => {
      html += `<li>${device.Name} (${
        device.DeviceId
      })<br>Last Message: ${moment(device.DateTime).format(
        "YYYY-MM-DD HH:mm"
      )}<br>Battery: ${device.Battery}<br>Signal: ${device.Lqi}</li>`;
    });
    html += `</ul>`;

    // Populate email template
    var emailHtml = fs
      .readFileSync(process.cwd() + "/templates/device-offline.html")
      .toString();
